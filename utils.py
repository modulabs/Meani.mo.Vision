...
class ResultWriter:
    ...
    def __init__(self, args):
        ...
        dataSetStr = os.path.basename(os.path.normpath(
            self.common_args['drone_folder_path']))
        self.dst_path = os.path.join(
            './', 'experiment' + str(self.common_args['experiment_num']) + '_' + dataSetStr)
        ...
        if self.common_args['save_video'] is True:
            ...
            self.vid = cv2.VideoWriter(
                video_dst, fourcc, 1, video_size, True, )
    ...
    def saveResults(self, result, idx):
        cv2.imwrite(os.path.join(self.dst_path, 'result_' +
                                 str(idx) + '.jpg'), result['image'])
        ...
        if self.common_args['save_masked_pcl'] is True:
            ...
            cv2.imwrite(os.path.join(self.dst_path,
                                     'masked_pcl.jpg'), result['masked_pcl'])

        if self.common_args['save_masked_drone'] is True:
            print("save_masked_drone")
            cv2.imwrite(os.path.join(self.dst_path, 'masked_drone_' +
                                     str(idx) + '.jpg'), result['masked_drone'])

        if self.common_args['save_matching'] is True:
            print("save_matching")
            cv2.imwrite(os.path.join(self.dst_path, 'matching1_' +
                                     str(idx) + '.jpg'), result['matching1'])
            cv2.imwrite(os.path.join(self.dst_path, 'matching2_' +
                                     str(idx) + '.jpg'), result['matching2'])
            cv2.imwrite(os.path.join(self.dst_path, 'matching3_' +
                                     str(idx) + '.jpg'), result['matching3'])

        if self.common_args['save_keypoints'] is True:
            print("save_keypoints")
            cv2.imwrite(os.path.join(
                self.dst_path, 'keypoints_lidar.jpg'), result['keypoints_lidar_image'])
            cv2.imwrite(os.path.join(self.dst_path, 'keypoints_drone_' +
                                     str(idx) + '.jpg'), result['keypoints_drone_image'])
