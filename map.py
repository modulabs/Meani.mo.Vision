...
def registration(drone_img_path, lidar_img_path, outout_file):
    ...
    drone_features, drone_descs = feature_detector.get_feature_and_descriptor(
        img_type="drone")
    lidar_features, lidar_descs = feature_detector.get_feature_and_descriptor(
        img_type="lidar")
...
if __name__ == '__main__':
    if (len(sys.argv) < 4):
        print >> sys.stderr, ("Usage: %s <image_dir> <key_frame> <output>" %
                              sys.argv[0])
        sys.exit(-1)
