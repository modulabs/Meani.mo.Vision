...
def main():
    ...
    lidar_image = lidar_image[lidar_roi_y:lidar_roi_y + lidar_roi_height, lidar_roi_x:lidar_roi_x + lidar_roi_width, :]
    ...
    resized_lidar = cv2.resize(lidar_image, dsize=None,fx=lidar_imshow_scale, fy=lidar_imshow_scale)
    resized_drone = cv2.resize(drone_image, dsize=None,fx=drone_imshow_scale, fy=drone_imshow_scale)
    ...
    while(1):
        ...
        if lidar_pos_change == True:
            lidar_roi_image = lidar_image[lidar_y1:lidar_y1+lidar_roi_width_height*2,lidar_x1:lidar_x1+lidar_roi_width_height*2:]
            print(lidar_roi_width_height)
            lidar_pos_change = False 

        if drone_pos_change == True:
            drone_roi_image = drone_image[drone_y1:drone_y1+drone_roi_width_height*2,drone_x1:drone_x1+drone_roi_width_height*2:]
            drone_pos_change = False 

        cv2.imshow('lidar_roi_image',cv2.resize(lidar_roi_image, dsize=(250,250)))
        cv2.imshow('drone_roi_image',cv2.resize(drone_roi_image, dsize=(250,250)))
