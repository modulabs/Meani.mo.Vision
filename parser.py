...
def make_parser():
    ...
    # Binary Mask
    parser.add_argument('--pcl_mask', default=True, help='Applying binary mask to a pcl image')
    parser.add_argument('--drone_mask', default=False, help='Applying binary mask to a drone image')

    # RANSAC (see https://docs.opencv.org/3.4.0/d9/d0c/group__calib3d.html#ga4abc2ece9fab9398f2e560d53c8c9780)
    parser.add_argument('--ransac_maxIters', default=2000, type=int, help='The maximum number of RANSAC iterations, 2000 is the maximum it can be.')
    parser.add_argument('--ransac_confidence', default=0.995, type=float, help='Confidence level, between 0 and 1 (default:0.995).')
    ...
    parser.add_argument('--SIFT_nOctaveLayers', default=3, type=float, help='The number of layers in each octave. 3 is the value used in D (default=3).')
    ...
    parser.add_argument('--debug', default=True, help='Imshow Intermediate Results')
    parser.add_argument('--save_video',default=True, help='Record result video for making GIF')
    parser.add_argument('--save_csv', default=True, help='Save csv')
    parser.add_argument('--save_masked_pcl', default=True, help='Save masked pcl image')
    parser.add_argument('--save_masked_drone', default=True, help='Save masked drone image')
    parser.add_argument('--save_matching', default=True, help='Save matching images')
    parser.add_argument('--save_keypoints', default=True, help='Save keypoints images')
